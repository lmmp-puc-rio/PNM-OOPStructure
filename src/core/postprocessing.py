...
class PostProcessing:
    ...
    def plot_network(self, lwidth=3, msize=100, azim=-60, elev=15):
        ...
        Np_col = len(np.unique(pn.coords.T[0]))
        Np_row = len(np.unique(pn.coords.T[1]))
        pn_dim = '2D'
        ...
        op.visualization.plot_coordinates(pn, markersize=markersize, c=phase_ic_color, alpha=0.8, ax=ax0)
        op.visualization.plot_connections(pn, linewidth=linewidth, c=phase_ic_color, alpha=0.8, ax=ax0)
